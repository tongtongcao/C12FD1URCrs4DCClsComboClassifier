...
class ClusterDataset(Dataset):
    ...
    Each row of the input CSV is expected to have 12 features:
        ...
        - Next 2 columns: x, y for uRWell crosses (normalized by x, y ranges)\
        - Last 2 columns: superlayers of missing cluster
    ...
    def __init__(self, csv_files):
        ...
        # uRWell features (x, y)
        X_urwell = df.iloc[:, 8:10].values.astype(np.float32)
        X_urwell[:, 0] = (X_urwell[:, 0]) / 140.0  # x
        X_urwell[:, 1] = X_urwell[:, 1] / 160.0  # y

        X_superlayer = df.iloc[:, 10:12].values.astype(np.float32) / 6.0
